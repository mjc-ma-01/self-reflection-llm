...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Score generated Reflector answers.")
    parser.add_argument("eval_ds", help="Dataset name: xstest, strongreject, donot, simpleqa, gsm8k, wildchat, advglue")
    parser.add_argument("--input_path", type=Path, default=None)
    parser.add_argument("--output_dir", type=Path, default=MODEL_ANSWER_DIR)
    return parser
...
def main() -> None:
    ...
    input_path = args.input_path or MODEL_ANSWER_DIR / f"{eval_ds}.json"
    ...
    def process_dataset(current_data, suffix=""):
        ...
        if eval_ds == "xstest":
            output_path = output_dir / f"{eval_ds}{suffix}_check.json"
        else:
            output_path = output_dir / f"{eval_ds}_check.json"
        ...
        try:
            from .harmbench import predict
        except Exception as exc:
            raise RuntimeError(
                "HarmBench scoring requires transformers, torch, and bitsandbytes. "
                "Set RUN_SCORE=0 in scripts/sft/validate_sft.sh to only generate answers."
            ) from exc
