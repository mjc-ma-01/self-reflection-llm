...
from .paths import DATA_SRC_DIR, DRA_PROCESSED_DIR
...
class Math(Dataset):
    def __init__(self, num_samples=None) -> None:  # 添加num_samples参数
        self.num_samples = num_samples
        self.dataset = load_dataset("Satori-reasoning/Satori_FT_data")["train"]
...
class ReflectDataset:
    ...
    def get_dataset(self):
        ...
        math = Math(num_samples=200)
        m = math.get_dataset()

        alpaca = Alpaca(num_samples=400)
...
class Adv_GLUE(Dataset):
    def __init__(self, num_samples=None) -> None:  # 添加num_samples参数
        self.num_samples = num_samples
        self.dataset = load_dataset("AI-Secure/adv_glue", "adv_qnli")["validation"]
        breakpoint()
    def get_dataset(self):
